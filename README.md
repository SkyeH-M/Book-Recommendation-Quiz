...
| | After a user has answered all 10 questions the option buttons are disabled meaning their clicks are no longer registered, this prompts the user to click submit and receive their recommendation. |
...
2. Device dependent event handler for my modal close button, WAVE explains that this isn't accessible to both mouse and keyboard users. My quiz in it's entirety does rely on a user having a mouse or using a smartphone in order to select option buttons and to submit the quiz. Therefore, the device dependent event handler is something that I will address in the future but currently don't feel that it impacts user experience.
...
* Modal area:
  ...
  * A user is prevented from entering a username including more than 15 characters as the input field will stop accepting characters when 15 have been entered, there is no way for this to be avoided.
...
* Recommendation area:
  ...
  * I used Chrome Dev Tools to test whether the option buttons are always disabled after a user has answered 10 questions, once disabled the option buttons will not log any value or input to the genreArray and will not be registered as clicks.
* Site wide:
...
| 9. I realised before completing this project that if a user clicked on option buttons after having answered all 10 questions the following [error](./docs/bugs/bug9.png) would appear in Chrome Dev Tools. This didn't affect the user but I wanted to fix this | Yes | I added some logic so that once the genreArray array reached a length of 10, meaning the user had completed the quiz the option buttons would be disabled and no further clicks would be registered. This removed the error and hopefully made it clearer to users that they must now click the submit button |
| 10. I discovered that there was no limit to how many characters users could enter into the username field. This would mean that when being greeted by their name in the recommendation area a potentially large amount of characters would flood the page and create disharmony between other elements nearby | Yes | I added a username input field maxlength property set to limit a username to 15 characters |
| 11. After adding logic so that the option buttons are disabled after 10 questions have been answered I wanted to override the hover effect so that the option buttons would no longer turn light green when hovered over. Ideally this would have been another way to alert users that the quiz was now finished | No | I did manage to accomplish this specific goal but found that it impacted other functions and hover effects so due to my lack of knowledge currently I've had to move this aim to be a future implementation goal. |
...
* [How to specify min/max number of characters allowed in an input field](https://www.geeksforgeeks.org/how-to-specify-minimum-maximum-number-of-characters-allowed-in-an-input-field-in-html/) 
* [Disable a button in JS based on condition](https://linuxhint.com/disable-button-in-javascript-based-on-condition/#:~:text=To%20disable%20a%20button%20in%20JavaScript%20based%20on%20condition%2C%20utilize,()%E2%80%9D%20method%20with%20%E2%80%9Cdocument.) 
...
* Robert Mclaughlin was instrumental in answering questions and providing feedback throughout the development of this project and provided much needed support.
* Akshat Garg aided immensely in terms of refactoring my JavaScript code to improve readability and adhere more closely to industry standards of efficiency and best practise.
